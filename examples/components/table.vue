<template>
	<div id="demo-table" class='demo'>
		<h2>初始化</h2>
		<code>html:</code>
		<highlightjs language='html' :code="htmlCode"/>
		<code>js:</code>
		<highlightjs language='javascript' :code="jsCode"/>
		<code>结果:</code>
		<vb-table ref='table' :columns="columns" title="test" :init-front-page-data='frontPageData' :on-click-td-btn='onClickTdBtn' :page-size-arr="[5,10,50]" :init-page-size='5'></vb-table>
		<h2>props</h2>
		...
		<h2>data</h2>
		<vb-table :columns="propColumns" :pagination='false' :init-front-page-data="data"></vb-table>
	</div>
</template>
<script>
	...
	export default {
		data(){
			return {
				htmlCode : 
Util.formatCode(`<vb-table ref='table' :columns="columns" title="test" :init-front-page-data='frontPageData' :on-click-td-btn='onClickTdBtn'></vb-table>`),
			  jsCode : 
